...
extern "C" mat_cv *load_image_mat_cv(const char *filename, int flag)
{
    ...
    catch (...) { cerr << "OpenCV exception: load_image_mat_cv \n"; }
...
cv::Mat load_image_mat(char *filename, int channels)
{
    ...
    else fprintf(stderr, "OpenCV can't force load with %d channels\n", channels);

    cv::Mat *mat_ptr = (cv::Mat *) load_image_mat_cv(filename, flag);

    if (mat_ptr == NULL) return cv::Mat();
...
extern "C" mat_cv* get_capture_frame_cv(cap_cv *cap)
{
    ...
    try
    {
        mat = new cv::Mat();//new一个
        if (cap)
        {
            cv::VideoCapture &cpp_cap = *(cv::VideoCapture *)cap;//引用视频类

            //打开的状态下读取一帧图像
            if (cpp_cap.isOpened()) cpp_cap >> *mat;
...
extern "C" int wait_for_stream(cap_cv *cap, cv::Mat* src, int dont_close)
{
    ...
    if (!src)
    {
        //IP摄像头获取定期关闭了，我们创建一个空白帧
        if (dont_close) src = new cv::Mat(416, 416, CV_8UC(3));
        else return 0;
    }
...
extern "C" image get_image_from_stream_resize(cap_cv *cap, int w, int h, int c, mat_cv** in_img, int dont_close)
{
    ...
    if (once)//只执行一次机制
    {
        ...
        do
        {
            if (src) delete src;//删除视频帧
            src = (cv::Mat*)get_capture_frame_cv(cap);//从视频设备读取一帧图像
            if (!src) return make_empty_image(0, 0, 0);//如果读取失败就返回一个空的视频帧
        } while (src->cols < 1 || src->rows < 1 || src->channels() < 1);//读取失败就一直读取
        printf("Video stream: %d x %d \n", src->cols, src->rows);
    }
    else src = (cv::Mat*)get_capture_frame_cv(cap);//读取一帧

    //IP摄像头定期关闭了，我们需要等待它的开启
