    ...
    int net_w, int net_h)
{
    ...
    int count = 0;
    int i;
    box_label *boxes = read_boxes(labelpath, &count);//读取方框标注信息
    int min_w_h = 0;
    ...
    float jitter, float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed, int letter_box, int show_imgs)
{
    ...
    for (i_mixup = 0; i_mixup <= use_mixup; i_mixup++)
    {
        ...
        else random_paths = get_random_paths(paths, n, m);//随机打乱图片列表，防止学习同一个类型

        //遍历每一张图像
        for (i = 0; i < n; ++i)
        {
            float *truth = (float*)xcalloc(5 * boxes, sizeof(float));
            const char *filename = random_paths[i];//获取一张图片地址

            int flag = (c >= 3);
            mat_cv *src;
            src = load_image_mat_cv(filename, flag);//读取图像文件数据
            if (src == NULL)
            {
                if (check_mistakes)
                {
...
void *load_thread(void *ptr)
{
    ...
    } else if (a.type == DETECTION_DATA)//主要就是这里
    {
...
void *load_threads(void *ptr)
{
    ...
    if (args.threads == 0) args.threads = 1;//最少创建一个读取图片文件线程
    data *out = args.d;
    int total = args.n;
    free(ptr);
    data* buffers = (data*)xcalloc(args.threads, sizeof(data));//每一个线程读取一张图片数据
    pthread_t* threads = (pthread_t*)xcalloc(args.threads, sizeof(pthread_t));//线程句柄
    ...
    for(i = 0; i < args.threads; ++i) pthread_join(threads[i], 0);//等待每一个线程都执行完毕
    *out = concat_datas(buffers, args.threads);//将图像数据合并
    out->shallow = 0;
    for(i = 0; i < args.threads; ++i)//释放内存
    {
        buffers[i].shallow = 1;
        free_data(buffers[i]);
    }
