...
#endif
    int i;
    for(i = net.n-1; i > 0; --i) if(net.layers[i].type != COST) break;//从后面查起，不是损失层就返回
    return net.layers[i].output;//该层的输出
}
...
float *network_predict(network net, float *input)
{
    ...
    float *out = get_network_output(net);//获取最后yolo层
    return out;
}
...
detection *make_network_boxes(network *net, float thresh, int *num)
{
    ...
    for (i = 0; i < nboxes; ++i)
    {
        ...
        // tx,ty,tw,th uncertainty 中心点和宽度高度
...
void fuse_conv_batchnorm(network net)
{
    ...
    for (j = 0; j < net.n; ++j)
    {
        ...
        {
            //
            if (l->share_layer != NULL)  l->batch_normalize = 0;
            ...
            if (l->batch_normalize)
            {
                ...
                l->batch_normalize = 0;//置0
#ifdef GPU
                if (gpu_index >= 0) push_convolutional_layer(*l);
#endif
            }
        }
        else  if (l->type == SHORTCUT && l->weights && l->weights_normalizion)//shortcut层 有权重 有batch
        {
            //显示权重
            if (l->nweights > 0)
            {
...
#ifdef GPU
            if (gpu_index >= 0)  push_shortcut_layer(*l);
#endif
        }
        else {
    ...
    for (j = 0; j < net.n; ++j)
    {
        ...
        if (l->type == CONVOLUTIONAL)
        {
            if (l->xnor)//异或标志
            {
                ...
                if (net.layers[j].use_bin_output) l->activation = LINEAR;
#ifdef GPU
