...
#endif
        nets[k] = parse_network_cfg(cfgfile);//读取cfg文件的层配置
        nets[k].benchmark_layers = benchmark_layers;
        if (weightfile) load_weights(&nets[k], weightfile);//读取权重文件
...
#endif
        if (avg_loss < 0 || avg_loss != avg_loss) avg_loss = loss;    // if(-inf or nan) 防止nan
        avg_loss = avg_loss*.9 + loss*.1;//计算损失
    ...
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    ...
    while (1)
    {
        ...
        if (!dont_show)  show_image(im, "predictions");
